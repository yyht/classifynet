...
from model.utils.re_augument import re_augument_utils
import tensorflow as tf
import numpy as np
...
class ReAugument(ModelTemplate):
    def __init__(self):
        super(ReAugument, self).__init__()
    ...
    def build_encoder(self, input_lengths, input_mask, *args, **kargs):
        ...
        with tf.variable_scope(self.config.scope+"_input_highway", reuse=reuse):
            ...
            [sent_repres_fw, sent_repres_bw, sent_repres_] = layer_utils.my_lstm_layer(sent_repres, 
                            self.config.context_lstm_dim, 
            ...
            if self.config.with_encoder_resduial:
                sent_repres = tf.layers.dense(sent_repres_, match_dim,
                            activation=tf.nn.relu) + sent_repres
            else:
                sent_repres = sent_repres_

            if self.config.multi_head:
                mask = tf.cast(input_mask, tf.float32)
                ignore_padding = (1 - mask)
                ignore_padding = re_augument_utils.attention_bias_ignore_padding(ignore_padding)
                encoder_self_attention_bias = ignore_padding

                sent_repres = re_augument_utils.multihead_attention_texar(
                    sent_repres, 
                    memory=None, 
        ...
        with tf.variable_scope(self.config.scope+"sent_label_attention", reuse=reuse):
            ...
            output = re_augument_utils.memory_attention_v2(
                    sent_repres, self.memory, 
            ...
            output = match_utils.multi_highway_layer(output, match_dim, 
    ...
    def build_predictor(self, output,
                        *args, **kargs):
        reuse = kargs["reuse"]
        predictor_type = kargs["predictor_type"]
        ...
        with tf.variable_scope(self.config.scope+"_prediction_module", reuse=reuse):
            #========Prediction Layer=========
            match_dim = output.get_shape()[-1]
            matched_repres = tf.nn.dropout(output, (1 - dropout_rate))
            self.classify_w = tf.get_variable("w_0", [match_dim, 
                                            self.num_classes], 
                                            dtype=tf.float32)
            self.classify_bias = tf.get_variable("b_0", [self.num_classes], dtype=tf.float32)

            if predictor_type == "main_part":
                logits = tf.einsum("abc,cb->ab", matched_repres, self.classify_w)
                logits += self.classify_bias

            elif predictor_type == "class_part":
                logits = tf.matmul(matched_repres, self.classify_w) + self.classify_bias
