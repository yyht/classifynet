from model.utils.biblosa.general import get_last_state, exp_mask_for_high_rank, mask_for_high_rank
from model.utils.biblosa.nn import linear, get_logits, pooling_with_mask, softsel, feature_combination, dropout,\
    bn_dense_layer
from model.utils.biblosa.rnn import SwitchableDropoutWrapper
from model.utils.biblosa.rnn import dynamic_rnn, bidirectional_dynamic_rnn
import tensorflow as tf
import math
from model.utils.biblosa.general import add_reg_without_bias, exp_mask
...
def directional_attention_with_dense(
    ...
    with tf.variable_scope(scope or 'directional_attention_%s' % direction or 'diag'):
        ...
        with tf.variable_scope('attention'):  # bs,sl,sl,vec
            ...
            dependent = linear(rep_map_dp, ivec, False, scope='linear_dependent', is_train=is_train)  # bs,sl,vec
            dependent_etd = tf.expand_dims(dependent, 1)  # bs,1,sl,vec
            head = linear(rep_map_dp, ivec, False, scope='linear_head', is_train=is_train) # bs,sl,vec
