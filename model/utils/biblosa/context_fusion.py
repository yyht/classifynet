...
from model.utils.biblosa.rnn import contextual_bi_rnn
from model.utils.biblosa.cnn import cnn_for_context_fusion, cnn_for_sentence_encoding, hierarchical_cnn_res_gate
from model.utils.biblosa.self_attn import multi_head_attention, multi_head_attention_git,\
    directional_attention_with_dense, bi_directional_simple_block_attention, multi_dimensional_attention
from model.utils.biblosa.nn import bn_dense_layer
...
def context_fusion_layers(
    ...
    with tf.variable_scope(scope or 'context_fusion_layers'):
        if method in ['lstm', 'gru', 'sru_normal', 'sru']:
            ...
            num_layers = kwargs["config"].num_slstm_layer
            rep_len = tf.reduce_sum(rep_mask, axis=-1)
            context_fusion_output = slstm_cell(kwargs["config"], "slstm", 
                        hn, 
                        rep_len, rep_tensor, rep_tensor, num_layers,
                        1 - dropout, reuse=None)
        else:
            raise RuntimeError
