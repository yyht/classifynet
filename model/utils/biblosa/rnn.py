...
from model.utils.biblosa.general import flatten, reconstruct
from model.utils.biblosa.nn import bn_dense_layer
from tensorflow.contrib.rnn import DropoutWrapper
from model.utils.biblosa.general import get_last_state, add_reg_without_bias
...
class NormalSRUCell(tf.contrib.rnn.RNNCell):
    def __init__(self, num_units, activation=None, keep_prob=1.0, is_train=False,
            reuse=None):
    ...
    with tf.variable_scope(scope or 'contextual_bi_rnn'): # correct
        ...
        if cell_type == 'sru':
            ...
            if cell_type == 'gru':
                ...
                cell_fw = NormalSRUCell(hn, keep_prob=keep_prob, is_train=is_train, reuse=reuse)
                cell_bw = NormalSRUCell(hn, keep_prob=keep_prob, is_train=is_train, reuse=reuse)
            else:
