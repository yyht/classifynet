...
def my_lstm_layer(input_reps, lstm_dim, input_lengths=None, scope_name=None, reuse=False, is_training=True,
    ...
    with tf.variable_scope(scope_name, reuse=reuse):
        ...
        if use_cudnn:
            ...
            batch_size = tf.shape(input_reps)[0]
            input_size = tf.shape(input_reps)[-1]

            rnn = rnn_model.BiCudnnRNN(lstm_dim, batch_size, input_size,
            ...
            f_rep = res[:, :, 0:lstm_dim]
            b_rep = res[:, :, lstm_dim:2*lstm_dim]
            outputs = res
        else:
            if lstm_type == "normal":
                ...
                context_lstm_cell_fw = tf.contrib.rnn.LayerNormBasicLSTMCell(lstm_dim)
                context_lstm_cell_bw = tf.contrib.rnn.LayerNormBasicLSTMCell(lstm_dim)
