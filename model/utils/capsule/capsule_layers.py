...
from model.utils.capsule import capsule_utils
...
def squash_v1(x, axis=-1):
    s_squared_norm = tf.reduce_sum(tf.square(x), axis, keepdims=True) + 1e-10
    scale = tf.sqrt(s_squared_norm)/ (0.5 + s_squared_norm)
    return scale * x

def squash_v0(s, axis=-1, epsilon=1e-7, name=None):
    s_squared_norm = tf.reduce_sum(tf.square(s), axis, keepdims=True) + 1e-10
...
#            tf.transpose(tf.transpose(c, perm=[0,2,1]) * i_activations, perm=[0,2,1]) 
        outputs = squash_v1(capsule_utils.batch_dot(c, u_hat_vecs, [2, 2]))
        if i < iterations - 1:
            b = b + capsule_utils.batch_dot(outputs, u_hat_vecs, [2, 3])                                    
    poses = outputs 
...
def capsules_init(inputs, shape, strides, padding, pose_shape, add_bias, name):
    with tf.variable_scope(name):   
        poses = capsule_utils._conv2d_wrapper(
          inputs,
        ...
        activations = tf.sqrt(tf.reduce_sum(tf.square(poses), axis=-1)) + beta_a        
